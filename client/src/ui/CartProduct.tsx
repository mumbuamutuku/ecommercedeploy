...
const CartProduct = ({ product }: {product: ProductProps}) => {
  ...
  const handleRemoveProduct=() => {
    if (product) {
        ...
        toast.success(`${product?.name.substring(0, 20)} 
    ...
    <div className="flex py-6 sm:py-10">
        <Link to= {`/product/${product?._id}`}>
          <img src={product?.images[0]} alt="productImage" 
            className="h-24 w-24 rounded-md object-cover sm:h-48
            sm:w-48 border border-skyText/30 hover:border-skyText
            duration-300"
          />
        </Link>
        <div className="ml-4 flex flex-1 flex-col justify-between
          sm:ml-6">
            <div className="relative pr-9 sm:grid sm:grid-cols-4 sm:gap-x-6
              sm:pr-0">

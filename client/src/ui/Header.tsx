...
const Header = () => {
  ...
  return (
    <div className="w-full bg-whiteText md:sticky md:top-0 z-50">
      <div className="max-w-screen-xl mx-auto h-20 
      flex items-center justify-between px-4 lg:px-0">
        ...
        <div className="hidden md:inline-flex max-w-3xl w-full relative">
          ...
          ring-1 ring-insert ring-gray-300 placeholder:text-gray-400
          placeholder:font-normal focus:ring-1 focus:ring-darkText
          sm:text-sm px-4 py-2" />
          {searchText ? (
          <IoClose onClick={() => setSearchText("")} 
          className="absolute top-2.5 right-4 text-xl
          hover:text-red-500 cursor-pointer duration-200" />
            ): (
            <IoSearchOutline 
          className="absolute top-2.5 right-4 text-xl" /> 
          )}
        ...
        {searchText && (
          <div className="absolute left-0 top-20 w-full mx-auto max-h-[500px]
          px-10 py-5 bg-white z-20 overflow-y-scroll text-black shadow-lg 
          shadow-skyText scrollbar-hide">
            {filteredProducts.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 items-center justify-center">
                {filteredProducts?.map((item:ProductProps) => (
                  <ProductCard key={item?._id} item={item} setSearchText={setSearchText} />
                ))}
              </div> 
            ) : (
              <div className="py-10 bg bg-gray-50 w-full flex items-center
        ...
        {/**MenuBar */}
        <div className="flex items-center gap-x-6 duration-200">
          <Link to={'/profile'}>
          {currentUser? (
      ...
      <div className="w-full bg-darkText text-whiteText">
        <Container className="py-2 max-w-4xl flex items-center 
        gap-5 justify-between">
          <Menu>
            ...
            <Transition
              ...
              <MenuItems
                ...
                border-white/5 bg-black p-1 text-sm/6
                text-gray-300 [--anchor-gap:var(--spacing-1)] 
              ...
              {categories.map((item: CategoryProps) => (
                  <MenuItem key={item?._id}>
                    <Link
                      ...
                      py-2 px-3 data-[focus]:bg-white/20 tracking-wide"
                    >
                      <img
                        src={item?.image}
                        alt="categoryImage"
          ...
          </Menu>
          {
            bottomNavigation.map(({title,link})=>(
              <Link
              to={link} 
               key={title} className="uppercase hidden
